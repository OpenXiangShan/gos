...
static struct riscv_iommu_dc *riscv_iommu_get_dc(struct riscv_iommu *iommu, int dev_id)
{
...
static int riscv_iommu_enable(struct riscv_iommu *iommu)
{
	...
	case DDTP_MODE_3LEVEL:
		...
		ddtp = (((unsigned long)iommu->ddtp >> 2) & DDTP_PPN_MASK) | iommu->ddt_mode; 
		break;
	default:
		return -1; 
	}		
			       ...
			       unsigned int size, unsigned int gfp)
{
	...
	struct riscv_iommu_device *iommu_dev = (struct riscv_iommu_device *)dev->iommu.priv_data;
...
static unsigned long riscv_iommu_atp(struct riscv_iommu_device *iommu)
{
	...
	if (iommu->mode != RISCV_IOMMU_DC_FSC_MODE_BARE)
		atp |= (((unsigned long)iommu->pgdp) >> PAGE_SHIFT) & ATP_PPN_MASK;
...
static unsigned long riscv_iommu_iova_to_phys_with_devid(int dev_id, unsigned long iova)
{
	struct riscv_iommu_dc *dc;
...
{
	struct riscv_iommu_priv_data *priv = (struct riscv_iommu_priv_data *)data;
	...
	print("%s -- base:0x%x, cmdq_len:%d, fltq_len:%d, priq_len:%d, ddt_mode:%d\n", 
		__FUNCTION__, iommu.base, iommu.cmdq_len, 
		iommu.fltq_len, iommu.priq_len,
		iommu.ddt_mode);
