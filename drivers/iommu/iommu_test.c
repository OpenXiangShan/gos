...
int iommu_test(struct device *dev, void *data)
{
	...
	for (int i = 0; i < TEST_NUM;i++) {
		...
		print("%s dev_id:%d -- iova_to_phys, iova:0x%x, addr:0x%x\n", __FUNCTION__, dev->iommu.dev_id, iova, dma_iova_to_phys(dev, iova));	
		print("%s dev_id:%d -- iova_to_phys_with_devid, iova:0x%x, addr:0x%x\n", __FUNCTION__, dev->iommu.dev_id, iova, dma_iova_to_phys_with_devid(dev->iommu.dev_id, iova));	
		print("\n");
	}
