...
static int cmd_c2c_cas_lat_handler(int argc, char *argv[], void *priv)
{
	...
	for (i = 0; i < nr_cpu; i++) {
		...
		for (j = 0; j < nr_cpu; j++) {
			...
			int duration[nr_batch];
			int dura_diff = 0;
			int dura_ava;
			int dura_tot = 0;
			for (k = 0; k < nr_batch; k++) {
				create_task(names[i], do_core2core_cas,
					    &task_args[0], i, NULL, 0, NULL);
				create_task(names[j], do_core2core_cas,
					    &task_args[2], j, NULL, 0, NULL);

				sem_wait();
				sem_wait();

				duration[k] =
				    (duration_cpus[i] + duration_cpus[j]) / 2;
				dura_tot += duration[k];
			}

			dura_ava = dura_tot / nr_batch;

			for (k = 0; k < nr_batch; k++) {
				int diff =
				    duration[k] >
				    dura_ava ? duration[k] -
				    dura_ava : dura_ava - duration[k];
