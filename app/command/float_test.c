...
static int float_start2(void* data)
{
	int ret;
	set_f(0x2345);
	while(1){
		ret = read_f(0x2,0x2345);
		if(ret !=0){
			print("float_test2 is ERROR\n");
		}else{
			print("float_test2 is ok \n");
		}
	}
	return 0;
}

static int float_start3(void* data)
{
	int ret;
	set_f(0x4567);
	while(1){
		ret = read_f(0x3,0x4567);
		if(ret !=0){
			print("float_test3 is ERROR\n");
		}else{
			print("float_test3 is ok \n");
		}
	}
	return 0;
}

static int cmd_float_test_handler(int argc, char *argv[], void *priv)
{

	create_task("float_t1", float_start1, NULL, 1, NULL, 0, NULL);
	create_task("float_t2", float_start2, NULL, 1, NULL, 0, NULL);
	create_task("float_t3", float_start3, NULL, 1, NULL, 0, NULL);
...
static const struct command cmd_float_test = {
	.cmd = "float_test",
	.handler = cmd_float_test_handler,
	.priv = NULL,
};
