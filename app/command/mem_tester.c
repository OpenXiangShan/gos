...
int test_8bit_wide_random(unsigned long volatile *bufa,
			  unsigned long volatile *bufb, size_t count);
int test_16bit_wide_random(unsigned long volatile *bufa,
			   unsigned long volatile *bufb, size_t count);
#endif
...
static ul physaddrbase = 0;

void sdram_tester(void *aligned, uint32_t bufsize)
{
	ul loops, loop, i;
	...
	print("memtester version " __version__ " (%d-bit)\n", UL_LEN);
	print("Copyright (C) 2001-2020 Charles Cazabon.\n");
	print
	...
	for (i = 0; i < count; i++, p1++, p2++) {
		if (*p1 != *p2) {
			if (use_phys) {
				...
				print("FAILURE: " FMT_TARGET " != " FMT_TARGET
				       " at physical address " FMT_TARGET ".\n",
				       (ul) * p1, (ul) * p2, physaddr);
			} else {
				print("FAILURE: " FMT_TARGET " != " FMT_TARGET
				       " at offset " FMT_TARGET ".\n",
				       (ul) * p1, (ul) * p2,
				       (ul) (i * sizeof(ul)));
			}
		...
		for (i = 0; i < count; i++, p1++) {
			if (*p1 != (((j + i) % 2) == 0 ? (ul) p1 : ~((ul) p1))) {
				if (use_phys) {
					...
					print
					    ("FAILURE: possible bad address line at physical address "
					     FMT_TARGET ".\n", physaddr);
				} else {
					print
...
int test_random_value(ulv * bufa, ulv * bufb, size_t count)
{
	...
	for (i = 0; i < count; i++) {
		*p1++ = *p2++ = rand_ul();
		if (!(i % PROGRESSOFTEN)) {
			print("%c", '\b');
			print("%c", progress[++j % PROGRESSLEN]);
			fflush(stdout);
		}
	}
	print("\b \b");
...
#endif
void print_mem(unsigned long address, unsigned int num)
{
	print("address 0x=%lx, num=0x%0x \r\n", address, num);
}

static int cmd_mem_tester_handler(int argc, char *argv[], void *priv)
{
	...
	start = atoi(argv[0]);
	start = (start >> 2) << 2;
	if (!(mem_range_is_free(start, len))) {
		print("please use unused mem:\n");
		walk_unused_mem_and_print();
		return 0;
	}
	address = phy_to_virt(start);
	len = atoi(argv[1]);

	sdram_tester((void *)address, len);

	return 0;
}

static const struct command cmd_mem_tester = {
	.cmd = "mem_tester",
	.handler = cmd_mem_tester_handler,
	.priv = NULL,
};

int cmd_mem_tester_init()
{
	register_command(&cmd_mem_tester);

	return 0;
}

APP_COMMAND_REGISTER(mem_tester, cmd_mem_tester_init);
