...
static unsigned long gpa_address_start = 0x90000000;
...
void *gpa_alloc(struct device *dev, unsigned int size)
{
	...
	void *addr = (void *)gpa_address_start;
	unsigned long *maps;
	 ...
	 */
	while (index < TOTAL_GPA_PAGE_NUM) {
...
void gpa_free(struct device *dev, void *addr, unsigned int size)
{
	...
	index = ((unsigned long)addr - gpa_address_start) / PAGE_SIZE;
	if (index >= TOTAL_GPA_PAGE_NUM)
		return;
