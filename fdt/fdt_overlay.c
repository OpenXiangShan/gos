			      ...
			      int fragment_offset, char const **pathp)
{
	...
	if (!phandle) {
		...
		path =
		    fdt_getprop(fdto, fragment_offset, "target-path",
				&path_len);
		if (path)
						...
						int fixup_node, uint32_t delta)
{
	...
	fdt_for_each_property_offset(fixup_prop, fdto, fixup_node) {
		...
		for (i = 0; i < fixup_len; i++) {
			 ...
			 */
			memcpy((char *)&adj_val, (char *)(tree_val + poffset),
			       sizeof(adj_val));
				 ...
				 int property)
{
	...
	do {
		...
		poffset = strtoul((char *)(sep + 1), (char *)&endptr, 10);
	...
	fdt_for_each_property_offset(prop, fdto, ov_sym) {
		...
		if (len > 1) {	/* target is not root */
			if (!target_path) {
				...
				memcpy(buf, (char *)target_path, len + 1);

		} else
			len--;

		buf[len] = '/';
		memcpy(buf + len + 1, (char *)rel_path, rel_path_len);
